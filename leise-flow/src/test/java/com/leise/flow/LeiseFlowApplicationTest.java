...
public class LeiseFlowApplicationTest {
    ...
    public void testSingleQueryAction() {
        ...
        // initContext(context);
        singleQueryAction
                .setQuerySql("select user_name as userName, card_no as cardNo from user_info where user_id = :userId and user_id =:userId");
        singleQueryAction.setOutParamNames("cardNo");
        long startTime = System.currentTimeMillis();
        singleQueryAction.execute(context);
        long endTime = System.currentTimeMillis();
        System.out.println("cost:"
                + new BigDecimal(endTime - startTime).divide(new BigDecimal(1000), 4, RoundingMode.HALF_UP)
                + " seconds");
    }
    ...
    public void testListQueryAction() {
        ...
        FlowContext context = new FlowContext("leise-provider_provider", "queryFlowList", "查询流程列表", "1.0.0");
        listQueryAction
                .setQuerySql("select user_id as userId, user_name as userName, card_no as cardNo from user_info");
    ...
    public void testPageQueryAction() {
        ...
        pageQueryAction
                .setPageSql("select corp_id as corpId, corp_name as corpName,regist_no as registNo from corp_info where corp_name = :corpName limit :limit, :offset");
